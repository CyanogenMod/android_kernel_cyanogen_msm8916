...
{
     	int32_t rc;
        if (first_time_power_up_ov5648 && frontCameraProbeInfo()) {
	 	printk("do not probe ov5648,just return\n");
        	rc = -1;
        	return rc;
	}
	...
	rc = msm_sensor_power_up(s_ctrl);
    	if (0 == rc) {
       		printk("ov5648 probe ok,set first_time_power_up_ov5648 to 0.\n");
...
int32_t ov5648_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
...
#if 0
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client, 0x4202, 0xf,
		MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0) {
		printk("%s:write status failed of reg(0x4202) is %d\n", __func__, rc);
	}
	...
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(s_ctrl->sensor_i2c_client, 0x3018, &rdata,
		MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0) {
		printk("%s:read status failed of reg(0x3018) is %d\n", __func__, rc);
	}
	...
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client, 0x3018, rdata,
		MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0) {
		printk("%s:write status failed of reg(0x3018) is %d\n", __func__, rc);
	}

	msleep(40);
#if 0
	rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(s_ctrl->sensor_i2c_client, 0x100, 0x0,
		MSM_CAMERA_I2C_BYTE_DATA);
	if (rc < 0) {
		printk("%s:write status failed of reg(0x100) is %d\n", __func__, rc);
	}
