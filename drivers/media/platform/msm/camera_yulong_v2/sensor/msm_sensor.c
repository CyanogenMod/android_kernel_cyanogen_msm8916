...
#define CONFIG_MSMB_CAMERA_DEBUG 1 //opened by congpeichao
#undef CDBG
...
uint16_t  flag_read_closeloop = 0; //0;
uint16_t closeloop_HallOff = 0x80; //0x0D;
uint16_t closeloop_HallBiase = 0x40; //0x09
uint16_t closeloop_HallMax = 0x0702; //1024;
uint16_t  closeloop_HallMin = 0x0303; //0;
bool front_prepare_otp_data = false;
bool rear_prepare_otp_data = false;

EXPORT_SYMBOL(front_camera_probe_ok);
EXPORT_SYMBOL(rear_camera_probe_ok);
...
{
#define M_CLOSELOOP 0	//test by congpeichao
	int rc = 0;
	uint16_t chipid = 0;
	struct msm_camera_i2c_client *sensor_i2c_client;
	struct msm_camera_slave_info *slave_info;
...
// cpc start 	
	#if M_CLOSELOOP
	if (chipid == 0x0214 && 0 == flag_read_closeloop /*s_ctrl->sensordata->sensor_init_params->position == 0*/) { //check if it is closeloop module, for IMX135 rear module
		num_byte = 13;
		addr = 0x00;
		...
		cci_ctrl.cfg.cci_i2c_read_cfg.addr_type = MSM_CAMERA_I2C_BYTE_ADDR; //client->addr_type;
		cci_ctrl.cfg.cci_i2c_read_cfg.data = buf;
		cci_ctrl.cfg.cci_i2c_read_cfg.num_byte = num_byte;
		rc2 = v4l2_subdev_call(cci_client->cci_subdev, core, ioctl, VIDIOC_MSM_CCI_CFG, &cci_ctrl);
		CDBG("%s line %d rc = %d\n", __func__, __LINE__, rc2);

		//printk("read EEPROM, addr = 0x%x\n", addr);
		for (i = 0; i < num_byte; i++) {
			printk("read EEPROM, Byte %d: 0x%x\n", i, buf[i]);
		}
		if (!buf) {
			kfree(buf);
		}
		...
		if (!(rc2 && (rc2 != -ENOIOCTLCMD)) && (((buf[5]<<8) + buf[6]) & 0xFFFF)!=0 && (((buf[7]<<8) + buf[8]) & 0xFFFF)!=0)
		{
			closeloop_HallOff   = buf[3];
			closeloop_HallBiase = buf[4];
			closeloop_HallMax = ((buf[5]<<8) + buf[6]) & 0xFFFF;
			closeloop_HallMin = ((buf[7]<<8) + buf[8]) & 0xFFFF;
			flag_read_closeloop = 1;
		}
		printk("get closeloop data from EEPROM: HallOff: 0x%x, HallBiase: 0x%x, HallMax: 0x%x, HallMin: 0x%x.\n", closeloop_HallOff, closeloop_HallBiase, closeloop_HallMax, closeloop_HallMin);
	} else {
		printk("non-closeloop module, chipid is 0x%x.\n", chipid);
	}
#else
	printk("test with non-closeloop module.\n");
#endif
...
int msm_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	       ...
	       if (rear_prepare_otp_data==false && (0 == s_ctrl->sensordata->sensor_info->position)) {
		        ...
		        if (s_ctrl->func_tbl->sensor_prepare_otp(s_ctrl))
	              ...
	              //printk("finish prepare_otp_data for front camera\n");
                    ...
                    printk("already prepare OTP data fot sensor(%s).\n", s_ctrl->sensordata->sensor_name);
	 } else {
              printk("No need to prepare OTP data fot sensor(%s).\n", s_ctrl->sensordata->sensor_name);
	 }
	...
	case CFG_POWER_UP:
		...
		if (s_ctrl->func_tbl->sensor_power_up) {
			...
			if(s_ctrl->module_id)
				...
				cdata->cfg.sensor_init_params.module_id = 0;
			pr_err("%s:%d case: CFG_POWER_UP, sensor(%s) state %d\n", __func__, __LINE__,
				 s_ctrl->sensordata->sensor_name, s_ctrl->sensor_state);
			//added by congpeichao end
		} else {
			rc = -EFAULT;
		}
	...
	case CFG_POWER_DOWN:
		...
		if (s_ctrl->func_tbl->sensor_power_down) {
			...
			pr_err("%s:%d case: CFG_POWER_DOWN sensor state %d\n", __func__, __LINE__,
				s_ctrl->sensor_state);
		} else {
			rc = -EFAULT;
		}
...
//extern uint8_t vendor_id_in_eeprom;
ssize_t camerainfo_read(struct file *file, char __user *buff, size_t count, loff_t *offp)
{
        ...
        printk(KERN_ERR "%s :%x %d count=%d f_ops=%d", __func__, s_ctrl->module_id, (int)(*offp), count, (int)file->f_pos);
...
bool rearCameraProbeInfo(void){
   printk("%s rear_camera_probe_ok = %d",__func__,rear_camera_probe_ok);
   return rear_camera_probe_ok;
}

bool frontCameraProbeInfo(void){
   printk("%s front_camera_probe_ok = %d",__func__,front_camera_probe_ok);
