...
/* Logging macro */
#define MSM_SENSOR_DRIVER_DEBUG 1
#undef CDBG
...
int32_t msm_sensor_driver_probe(void *setting)
{
	...
	if (!s_ctrl) {
		pr_err("failed: s_ctrl %p for camera_id %d\n", s_ctrl,
			slave_info->camera_id);
		rc = -EINVAL;
		goto FREE_SLAVE_INFO;
	}
	...
	if (!camera_info) {
		pr_err("failed: no memory slave_info %p\n", camera_info);
		goto FREE_POWER_DOWN_SETTING;
