...
#define CONFIG_MSM_CAMERA_DT_DEBUG 1
#undef CDBG
	...
	struct msm_camera_i2c_client *sensor_i2c_client)
{
	...
	if (ctrl->power_setting[0].use_system_power) { 
	        if(((0 == sensor_board_info->sensor_info->position) && rearCameraProbeInfo()) || 
                ((1 == sensor_board_info->sensor_info->position) && frontCameraProbeInfo())) {
                        printk("current sensor is probed,do not request reset,pwd,ldo,af_pwd gpios,just request mclk gpio\n");
                        
                        for (index = 0; index < ctrl->gpio_conf->cam_gpio_req_tbl_size; index++) {
                                printk("current gpio lable is :%s\n",ctrl->gpio_conf->cam_gpio_req_tbl[index].label);
                                if (strcmp(ctrl->gpio_conf->cam_gpio_req_tbl[index].label,"CAMIF_MCLK") == 0) {
                                        printk("request mclk gpio\n");
			...
			if (power_setting->seq_val >= CAM_VREG_MAX) {
				...
				goto power_up_failed;
                        ...
                        if ((power_setting->use_system_power) &&
                                ...
                                ((1 == sensor_board_info->sensor_info->position) && frontCameraProbeInfo()))) {
                                printk("current sensor use system power,and has probed ok,do not request verg now\n");
                        } else {
                                //added by congpeichao test start
                                CDBG("cpc 1-%s name = %s, type = %d\n", __func__,
                                ctrl->cam_vreg[power_setting->seq_val].reg_name,
                                ctrl->cam_vreg[power_setting->seq_val].type);
                                //addeb by congpeichao test end
...
// || (CONFIG_BOARD_CP9191) || defined (CONFIG_BOARD_CP9191_T00)	
#if defined (CONFIG_BOARD_CP8675_C00)||defined (CONFIG_BOARD_CP5953)
		ret = pinctrl_select_state(ctrl->pinctrl_info.pinctrl,
	...
	ctrl->cam_pinctrl_status = 0;
        if ((ctrl->power_setting[0].use_system_power)) {
                printk("current sensor use system power,so do not release gpios,just release mclk gpio\n");
                for (index = 0; index < ctrl->gpio_conf->cam_gpio_req_tbl_size; index++) {
                        printk("current gpio lable is :%s\n",ctrl->gpio_conf->cam_gpio_req_tbl[index].label);
                        if (strcmp(ctrl->gpio_conf->cam_gpio_req_tbl[index].label,"CAMIF_MCLK") == 0) {
                                printk("release mclk gpio\n");
			...
			if (!ctrl->gpio_conf->gpio_num_info->valid
				...
				continue;
                        ...
                        if (pd->use_system_power) {
                                ...
                                (((0 == sensor_board_info->sensor_info->position) && rearCameraProbeInfo()) ||
                                ((1 == sensor_board_info->sensor_info->position) && frontCameraProbeInfo()))) {
                                         printk("camera use Sx voltage,so not disable other voltages-GPIO\n");
                                } else { 
		...
		case SENSOR_VREG:
			...
			if (ps) {
                                if ((pd->use_system_power) &&
                                        (((0 == sensor_board_info->sensor_info->position) && rearCameraProbeInfo()) ||
                                        ((1 == sensor_board_info->sensor_info->position) && frontCameraProbeInfo()))) {
                                        printk("camera use Sx voltage,and sensor probe ok,so not disable voltages-VREG\n");
                                } else {                                
                                        msm_camera_config_single_vreg(ctrl->dev,
