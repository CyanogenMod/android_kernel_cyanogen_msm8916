...
static int get_rbatt(struct qpnp_bms_chip *chip, int soc, int batt_temp)
{
	...
	pr_debug("rbatt_capacitive_mohm=%d\n",chip->batt_data->rbatt_capacitive_mohm);
...
static void monitor_soc_work(struct work_struct *work)
{
	...
	if (!is_battery_present(chip)) {
		...
		if (chip->dt.cfg_use_voltage_soc) {
			...
			pr_debug("old calculated_soc =%d,chip->last_ocv_uv=%d,vbat_uv=%d\n",chip->calculated_soc,chip->last_ocv_uv,vbat_uv);
...
static int calculate_initial_soc(struct qpnp_bms_chip *chip)
{
	...
	pr_info("warm_reset=%d  shutdown_soc_invalid=%d shutdown_ocv=%d shutdown_soc=%d last_soc=%d calculated_soc=%d last_ocv_uv=%d\n",
			chip->warm_reset, shutdown_soc_invalid,
