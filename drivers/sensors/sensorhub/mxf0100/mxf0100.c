...
#include <linux/sensors/mxf0100.h>
...
#define MXT_MOTION_SENSING_T75        75
#define MXT_MOTION_GESTURE_T76        76
...
/* Define T75 MEM MAP */
#define MXT_T75_CTRL (0)
#define MXT_T75_ACCLRPTINT (9)
#define MXT_T75_ACCLRPTTHR (11)
#define MXT_T75_LINACRPTINT (13)
#define MXT_T75_LINACRPTTHR (15)
#define MXT_T75_GYRORPTINT (17)
#define MXT_T75_GYRORPTTHR (19)
#define MXT_T75_MAGRPTINT (21)
#define MXT_T75_MAGRPTTHR (23)
#define MXT_T75_TILTRPTINT (25)
#define MXT_T75_TILTRPTTHR (27)
#define MXT_T75_ROTVCRPTINT (29)
#define MXT_T75_ROTVCRPTTHR (31)
#define MXT_T75_TEMPRPTINT (33)
#define MXT_T75_TEMPRPTTHR (35)

/* Define T76 MEM MAP */
#define MXT_T76_CTRL (0)
#define MXT_T76_SHAKECTRL (1)
#define MXT_T76_SHAKEINTMIN (2)
#define MXT_T76_SHAKEINTMAX (4)
#define MXT_T76_SHAKETHR (6)
...
/* PROCI_MOTIONGESTURES_T76 messages */
#define MXT_T76_RPTTYPE_SHAKE        1
#define MXT_T76_RPTTYPE_STEP        2
#define MXT_T76_RPTTYPE_TAP        3

#define MXT_T76_RPTID_STEP        12
#define MXT_T76_RPTID_TAP        13
#define MXT_T76_RPTID_SHAKE        14
...
#define MXT_CAL_TIMEOUT        5000    /* msec */
#define MXT_RESET_TIMEOUT    3000    /* msec */
#define MXT_CRC_TIMEOUT        1000    /* msec */
#define MXT_FW_RESET_TIME    3000    /* msec */
#define MXT_FW_CHG_TIMEOUT    300    /* msec */
#define MXT_WAKEUP_TIME        160    /* msec */
#define MXT_REGULATOR_DELAY    20    /* msec */
//#define MXT_POWERON_DELAY    150    /* msec */
#define MXT_POWERON_DELAY    200    /* msec */
...
#define DEBUG_MSG_MAX        2000

/* add by Jay.HF */
#define MXT_SAMPLES_TIMES        20
...
struct motion_sensing_object_t75_config {
    u8 ctrl;
    u8 reserved[8];
    u16 acclrptint;
    u16 acclrptthr;
...
struct motion_gesture_object_t76_config {
    u8 ctrl;
} __packed;
...
struct mxt_data {
    ...
    u16 T76_address; /* add by Jay.HF */
    u8 T76_reportid;
    u8 T100_reportid_min;
    u8 T100_reportid_max;

    /* for calibration, add by Jay.HF*/
    u8 samples_cnt;
    u32 cal_status;
    struct mxt_axis bias;
    struct mxt_axis samples[MXT_SAMPLES_TIMES];
    atomic_t cal_enabled;
    struct completion cal_completion;
    ...
    struct motion_gesture_object_t76_config t76_config;

#ifdef MXT_HAS_EARLYSUSPEND
#ifdef CONFIG_FB
...
static struct mxt_data *mxt_data = NULL;
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
    ...
    /* reset, irq gpio info */
    pdata->gpio_reset = of_get_named_gpio_flags(np, "atmel,reset-gpio", 0, &flags);
    pdata->gpio_pwr   = of_get_named_gpio_flags(np, "atmel,pwr-gpio", 0, &flags);
    pdata->gpio_irq   = of_get_named_gpio_flags(np, "atmel,irq-gpio", 0, &flags);
    pdata->gyro_irq   = of_get_named_gpio_flags(np, "atmel,gyro-irq", 0, &flags);
    pdata->meg_irq    = of_get_named_gpio_flags(np, "atmel,meg-irq", 0, &flags);
    if (YL_CURT_PCB_VER == YL_PCB_VER_T1) {
        pdata->gpio_mode = of_get_named_gpio_flags(np, "atmel,mode-gpio", 0, &flags);
        printk("mode-gpio = %d\n", pdata->gpio_mode);
        if (pdata->gpio_mode < 0) {
            return -EINVAL;
        }
    }
    printk("irq-gpio = %d\n", pdata->gpio_irq);
    ...
    printk("gyro-irq = %d\n", pdata->gyro_irq);
    printk("meg-irq = %d\n", pdata->meg_irq);
    if (pdata->gpio_reset < 0 || pdata->gpio_irq < 0 ||
        pdata->gyro_irq < 0 || pdata->meg_irq < 0 ||
        pdata->gpio_pwr < 0) {
            return -EINVAL;
    }
#if 0
    ret = of_property_read_u32(np, "atmel,irq_flags", (unsigned int *)&pdata->irqflags);
    if (ret) {
        dev_err(dev, "Looking up %s property in node %s failed",
            "atmel,irq_flags", np->full_name);
        return -ENODEV;
    }
    //ret = of_property_read_string(np, "atmel,vdd_ana", &pdata->vdd_ana);
#endif
    return 0;
}
#endif

void mxt_gpio_init(struct mxt_data *data)
{
    gpio_request(data->pdata->gpio_pwr, "mxf-pwr");
    gpio_request(data->pdata->gpio_reset, "mxf-rest");
    gpio_request(data->pdata->gpio_irq, "mxf-irq");
    gpio_request(data->pdata->gyro_irq, "gyro-irq");
    gpio_request(data->pdata->meg_irq, "meg-irq");
    if (YL_CURT_PCB_VER == YL_PCB_VER_T1) {
        gpio_request(data->pdata->gpio_mode, "mxf-mode");
        gpio_direction_output(data->pdata->gpio_mode, 1);
    }
    gpio_direction_input(data->pdata->gyro_irq);
    gpio_direction_input(data->pdata->meg_irq);
    gpio_direction_input(data->pdata->gpio_irq);
}

static int mxt_rptintval_init(struct mxt_data *data)
{
    int ret = 0;
    u16 value = 0;
    ret = __mxt_write_reg(data->client, data->T75_address + MXT_T75_TEMPRPTINT, sizeof(value), &value);
    if (ret != 0) {
        printk("disable temperature intval failed, ret=%d\n", ret);
    }
...
retry_read:
    ...
    if (ret == 1) {
        ...
        ret = (ret < 0) ? ret : -EIO;
        dev_err(&data->client->dev, "i2c recv failed (%d) retry %d\n", ret,retry);
        if(retry--)
            goto retry_read;
    }
...
retry_write:
    ...
    if (ret == 1) {
        ...
        ret = (ret < 0) ? ret : -EIO;
        dev_err(&data->client->dev, "i2c send failed (%d) retry %d\n", ret,retry);
        if(retry--)
            goto retry_write;
    }
...
static int mxt_lookup_bootloader_address(struct mxt_data *data, u8 retry)
{
    ...
    dev_err(dev, "***** bootloader, addr :%02X\n", bootloader);
...
static int mxt_probe_bootloader(struct mxt_data *data, u8 retry)
{
    ...
    if (ret) {
        dev_err(dev, "***** bootloader failed\n");
        return ret;
    }

    dev_err(dev, "***** bootloader success\n");
...
{
    struct device *dev = &data->client->dev;
  struct mxt_config *dcfg = &data->init_cfg;
    int error;
...
static void mxt_proc_t6_messages(struct mxt_data *data, u8 *msg)
{
    ...
    if ((data->t6_status & MXT_T6_STATUS_RESET) &&
        ...
        dev_info(dev, "T6 msg complete soft reset\n");
...
static void mxt_proc_t75_sensor_messages(struct mxt_data *data, u8 *msg)
{
    int id;
    struct device *dev = &data->client->dev;
    ...
    id = msg[0] - data->T75_reportid_min;
    //input_report_abs(data->input_dev, ABS_MISC, 255); /* add by Jay.HF */
    input_event(data->input_dev, EV_MSC, MSC_SCAN, id);

    /* add by Jay.HF */
    if (data->debug_enabled) {
        dev_info(dev, "report-id is %d\n", id);
    }

#if defined(MXT_ACCEL_CALIBRATION_SUPPORT)
    /* add by Jay.HF */
    if (ACCEL == id) {
        input_report_abs(data->input_dev, ABS_X, (msg[1] | (msg[2] << 8)) + data->bias.x);
        input_report_abs(data->input_dev, ABS_Y, (msg[3] | (msg[4] << 8)) + data->bias.y);
        input_report_abs(data->input_dev, ABS_Z, (msg[5] | (msg[6] << 8)) + data->bias.z);
    }
    else {
        input_report_abs(data->input_dev, ABS_X, msg[1] | (msg[2] << 8));
        input_report_abs(data->input_dev, ABS_Y, msg[3] | (msg[4] << 8));
        input_report_abs(data->input_dev, ABS_Z, msg[5] | (msg[6] << 8));
    }
#else
    input_report_abs(data->input_dev, ABS_X, msg[1] | (msg[2] << 8));
    input_report_abs(data->input_dev, ABS_Y, msg[3] | (msg[4] << 8));
    ...
    if (atomic_read(&data->cal_enabled) &&
        ...
        data->samples[data->samples_cnt].z = msg[5] | (msg[6] << 8);
        if (++data->samples_cnt >= MXT_SAMPLES_TIMES) {
            atomic_set(&data->cal_enabled, 0);
            complete(&data->cal_completion);
        }
        dev_info(dev, "%s: sample times %d\n", __func__, data->samples_cnt);
    }
    /* end by Jay.HF */
#endif
}

static void mxt_proc_t76_motion_gesture(struct mxt_data *data, u8 *msg)
{
    switch (msg[1])
    {
    case MXT_T76_RPTTYPE_SHAKE:
        //input_report_abs(data->input_dev, ABS_MISC, MXT_T76_RPTID_SHAKE);
        input_event(data->input_dev, EV_MSC, MSC_SCAN, MXT_T76_RPTID_SHAKE);
        input_report_abs(data->input_dev, ABS_X, msg[2]);
        input_sync(data->input_dev);
        break;

    case MXT_T76_RPTTYPE_STEP:
        //input_report_abs(data->input_dev, ABS_MISC, MXT_T76_RPTID_STEP);
        input_event(data->input_dev, EV_MSC, MSC_SCAN, MXT_T76_RPTID_STEP);
    ...
    case MXT_T76_RPTTYPE_TAP:
        //input_report_abs(data->input_dev, ABS_MISC, MXT_T76_RPTID_TAP);
        input_event(data->input_dev, EV_MSC, MSC_SCAN, MXT_T76_RPTID_TAP);
        input_report_abs(data->input_dev, ABS_X, msg[2]);
        input_sync(data->input_dev);
        break;
    }
...
static int mxt_proc_message(struct mxt_data *data, u8 *message)
{
    ...
    } else if (report_id >= data->T75_reportid_min
           && report_id <= data->T75_reportid_max) {
        mxt_proc_t75_sensor_messages(data, message);
    } else if (report_id == data->T76_reportid) {
        mxt_proc_t76_motion_gesture(data, message);
    } else {
        dump = true;
    }
...
static irqreturn_t mxt_process_messages_t44(struct mxt_data *data)
{
    ...
    if (count == 0) {
        if (data->debug_enabled) {
            dev_info(dev, "Interrupt triggered but zero messages\n");
        }
        return IRQ_NONE;
    ...
    if (ret < 0) {
        dev_warn(dev, "ret < 0 Unexpected invalid message\n");
        return IRQ_NONE;
    }
...
static int mxt_check_retrigen(struct mxt_data *data)
{
    ...
    if (data->T18_address) {
        ...
        if (val & MXT_COMMS_RETRIGEN) {
            dev_info(&client->dev, "%s: line is %d\n", __func__, __LINE__);
            return 0;
        }
    }

    dev_info(&client->dev, "Enabling RETRIGEN workaround\n");
...
static int mxt_check_reg_init(struct mxt_data *data)
{
    ...
    if (data->T75_address) {
        ...
        if (data->T75_address < cfg_start_ofs) {
            dev_err(dev, "Bad T75 address, T75addr = %x, config offset %x\n",
                data->T75_address, cfg_start_ofs);
            ret = 0;
            goto release_mem;
        }
    ...
    if (!update_t38) {
        memcpy(config_mem + data->T38_address - cfg_start_ofs, data->init_cfg.t38.data,
                sizeof(data->init_cfg.t38.data));
    }
    byte_offset = 0;
...
static void mxt_free_object_table(struct mxt_data *data)
{
    ...
    data->T75_address = 0; /* add by Jay.HF */
    data->T75_reportid_min = 0;
    data->T75_reportid_max = 0;

    data->T76_address = 0; /* add by Jay.HF */
    data->T76_reportid = 0;
...
static int mxt_parse_object_table(struct mxt_data *data)
{
    ...
    for (i = 0; i < data->info->object_num; i++) {
        ...
        case MXT_PROCI_ACTIVE_STYLUS_T63:
            ...
            break;
        case MXT_MOTION_SENSING_T75:
            data->T75_address = object->start_address; /* add by Jay.HF */
            data->T75_reportid_min = min_id;
            data->T75_reportid_max = max_id;
            break;
        case MXT_MOTION_GESTURE_T76:
            data->T76_address = object->start_address; /* add by Jay.HF */
            data->T76_reportid = min_id;
            break;
...
static void mxt_enable(struct mxt_data *data)
{
    int ret = 0;
...
#if 0
    if (yl_phone_mode_is_xianwang()) { /* for xianwang */
        dev_info(&data->client->dev, "%s:xianwang\n", __func__);
        gpio_set_value(data->pdata->gpio_reset, 1);
        msleep(MXT_POWERON_DELAY);
        while (count++ < 15) {
            if (gpio_get_value(data->pdata->gpio_irq) == 0) {
                break;
            }
            msleep(MXT_REGULATOR_DELAY);
        }
        dev_info(&data->client->dev, "%s:count %d\n", __func__, count);
    }
#endif
    return;
}
...
{
    int ret = 0;
    struct motion_sensing_object_t75_config t75;
    struct motion_gesture_object_t76_config t76;

    dev_info(&data->client->dev, "%s\n", __func__);

    /* clear T75&T76 config */
    memset(&t75, 0, sizeof(t75));
    memset(&t76, 0, sizeof(t76));
...
#if 0
    if (yl_phone_mode_is_xianwang()) { /* for xianwang */
        dev_info(&data->client->dev, "%s:xiangwang\n", __func__);
...
static int mxt_initialize_maxfusion(struct mxt_data *data)
{
    struct device *dev = &data->client->dev;
    struct input_dev *input_dev;
    int ret;
    ...
    if (data->input_dev) {
        dev_err(dev, "maxfusion input is exsit\n");
        return 0;
    }
    /* Initialize input device */
    input_dev = input_allocate_device();
    if (!input_dev) {
        dev_err(dev, "Failed to allocate sensor device\n");
        return -ENOMEM;
    }

    input_dev->name = "atmel-maxfusion";
    ...
    input_set_abs_params(input_dev, ABS_MISC,
                 0, 255, 0, 0); /* modify by Jay.HF */
    input_set_abs_params(input_dev, ABS_THROTTLE,
                 0, 255, 0, 0);
    ...
    if (ret) {
        dev_err(dev, "Error %d registering sensor device\n", ret);
...
static int mxt_configure_objects(struct mxt_data *data)
{
    ...
    if (data->T9_reportid_min) {
        ...
        if (error)
            return error;
    } else if (data->T75_reportid_min) {
        error = mxt_initialize_maxfusion(data);
        if (error)
            return error;
    } else {
...
static int mxt_load_fw(struct device *dev)
{
    ...
    if (!data->in_bootloader) {
        ...
        dev_info(dev, "in bootloader mode\n");
        enable_irq(data->irq);
    }
...
#if 1
    /* Wait for flash */
    ret = mxt_wait_for_completion(data, &data->bl_completion, MXT_FW_RESET_TIME);
    if (ret) {
        dev_err(dev, "%s: fw reset failed\n", __func__);
        goto disable_irq;
    }
...
static int mxt_check_firmware_version(struct mxt_data *data, char *version_str)
{
    struct device *dev = &data->client->dev;
    ...
    dev_info(dev, "The new version is %s, current version is %s\n",
...
static int mxt_check_calibration_tag(struct mxt_data *data)
{
    struct device *dev = &data->client->dev;
    struct mxt_config *dcfg = &data->init_cfg;
    u32 mflag;
    ...
    mflag = dcfg->t38.data[2] | ((u32)dcfg->t38.data[1]<<8) | ((u32)dcfg->t38.data[0]<<16);
                    ...
                    const char *buf, size_t count)
{
    ...
    if (error) {
        error = mxt_check_calibration_tag(data);
        if (error)
            return error;
    }
...
static ssize_t mxt_gpio_ctrl_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    struct mxt_data *data = dev_get_drvdata(dev);
    int i, j, k;

    dev_info(dev, "buf is %s\n", buf);
    if (sscanf(buf, "%d %d %d", &i, &j, &k) == 3) {
        gpio_set_value(data->pdata->gpio_reset, !!i);
        gpio_set_value(data->pdata->gpio_pwr, !!j);
        if (k) {
            gpio_direction_input(data->pdata->gpio_irq);
        }
        else {
        ...
        dev_info(dev, "gpio write error\n");
        return -EINVAL;
    }
}

static ssize_t mxt_prevent_suspend_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    struct mxt_data *data = dev_get_drvdata(dev);
    int i;
    ...
    dev_info(dev, "buf is %s\n", buf);
    if (sscanf(buf, "%u", &i) == 1 && i < 2) {
        data->prevent_suspend = (i == 1);

        dev_info(dev, "prevent_suspend: %d\n", data->prevent_suspend);
        return count;
    } else {
        dev_info(dev, "prevent_suspend write error\n");
        return -EINVAL;
    }
}

static ssize_t mxt_msg_inject_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct mxt_data *data = dev_get_drvdata(dev);
    u8 message[8];
    ...
    ret = sscanf(buf, "%2hhx %2hhx %2hhx %2hhx %2hhx %2hhx %2hhx %2hhx",
            &message[0],
            &message[1],
            &message[2],
            &message[3],
    ...
    if (ret == 8) {
        mxt_proc_message(data, &message[0]);
    } else {
        dev_err(dev, "couldn't parse input\n");
    }
...
static DEVICE_ATTR(prevent_suspend, S_IWUSR | S_IRUSR, NULL, mxt_prevent_suspend_store);
static DEVICE_ATTR(gpio_ctrl, S_IWUSR | S_IRUSR, NULL, mxt_gpio_ctrl_store);
static DEVICE_ATTR(msg_inject, 0664, NULL, mxt_msg_inject_store);
...
static struct attribute *mxt_attrs[] = {
    ...
    &dev_attr_prevent_suspend.attr,
    &dev_attr_gpio_ctrl.attr,
    &dev_attr_msg_inject.attr,
    NULL
};
...
static void mxt_stop(struct mxt_data *data)
{
    ...
    if (data->T100_reportid_min || data->T9_reportid_min)
...
static int mxt_handle_pdata(struct mxt_data *data)
{
    ...
    if (data->client->dev.of_node) {
        ...
        if (error) {
            dev_err(&data->client->dev, "Failed to parse dt\n");
            kfree(data->pdata);
            return error;
        }
    }
...
static long mxt_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
    int ret = 0;
    struct mxt_data *data = mxt_data;
    struct SensorCalInfo cal_info;
#if defined(MXT_ACCEL_CALIBRATION_SUPPORT)
    int i, x_sum = 0, y_sum = 0, z_sum = 0;
#endif
    switch (cmd) {
        case MXT_IOCTL_CALIBRATE:
            printk(KERN_ERR"%s: do calibration\n", __func__);
            memset(&cal_info, 0, sizeof(cal_info));
        #if defined(MXT_ACCEL_CALIBRATION_SUPPORT)
            INIT_COMPLETION(data->cal_completion);
            data->samples_cnt = 0;
            atomic_set(&data->cal_enabled, 1);
            ret = mxt_wait_for_completion(data, &data->cal_completion, MXT_CAL_TIMEOUT);
            if (ret) {
            ...
            atomic_set(&data->cal_enabled, 0);
            for (i = 0; i < MXT_SAMPLES_TIMES; i++) {
                x_sum += data->samples[i].x;
                y_sum += data->samples[i].y;
                z_sum += data->samples[i].z;
            }
            data->bias.x = 0 - x_sum/MXT_SAMPLES_TIMES;
            data->bias.y = 0 - y_sum/MXT_SAMPLES_TIMES;
            data->bias.z = MXT_ACCEL_MAX - z_sum/MXT_SAMPLES_TIMES;

            cal_info.SensorCalStatus = MXT_ACCEL_CAL_SUCCESS;
            cal_info.value[0] = MXT_ACCEL_BIAS_MASK;
            cal_info.value[1] = data->bias.x;
            cal_info.value[2] = data->bias.y;
            cal_info.value[3] = data->bias.z;
        #else
            cal_info.SensorCalStatus = MXT_ACCEL_CAL_SUCCESS;
            cal_info.value[0] = MXT_ACCEL_BIAS_MASK;
            cal_info.value[1] = 0;
            ...
            data->cal_status = MXT_ACCEL_CAL_SUCCESS;
            printk(KERN_ERR"%s: do calibration success\n", __func__);
            break;

        case MXT_IOCTL_GETSTATUS:
            if (copy_to_user((u32 __user *)arg, &data->cal_status, sizeof(data->cal_status))) {
                ret = -EFAULT;
            }
            break;
...
static int mxt_open(struct inode *inode, struct file *file) {
    struct mxt_data *data = mxt_data;
    struct SensorCalInfo cal_info;
    ...
    if ((sizeof(cal_info) == sensparams_read_from_flash(SENSPARAMS_TYPE_ACCEL, \
        ...
        if (MXT_ACCEL_CAL_SUCCESS == cal_info.SensorCalStatus) {
            data->bias.x = cal_info.value[1];
            data->bias.y = cal_info.value[2];
            data->bias.z = cal_info.value[3];
        }
    }
    printk(KERN_ERR"%s: cal_status(%d), bias_x(%d), bias_y(%d), bias_z(0x%x)\n",
        __func__, data->cal_status, data->bias.x, data->bias.y, data->bias.z);
    return (0);
}
...
static struct file_operations mxt_fops = {
    .owner          = THIS_MODULE,
    .open           = mxt_open,
    .release        = mxt_release,
...
static struct miscdevice mxt_device = {
...
#ifdef MXT_HAS_EARLYSUSPEND
#ifdef CONFIG_FB
static void mxt_configure_sleep(struct mxt_data *data)
{
    int retval = 0;
...
{
    struct fb_event *evdata = msg;
    int *blank;
    struct mxt_data *data = container_of(self, struct mxt_data, fb_notif);
    ...
    dev_info(&data->client->dev, "%s\n", __func__);
    if (data->in_bootloader) {
        dev_info(&data->client->dev, "%s: in bootloader mode\n", __func__);
        return 0;
    }
...
static int mxt_i2c_probe(struct mxt_data *data) {
    int error = 0;
    ...
    mxt_rptintval_init(data); /* add by Jay.HF */

    error = sysfs_create_group(&data->client->dev.kobj, &mxt_attr_group);
    if (error) {
        dev_err(&data->client->dev, "Failure %d creating sysfs group\n", error);
        goto err_free_object;
    }

    sysfs_bin_attr_init(&data->mem_access_attr);
    data->mem_access_attr.attr.name = "mem_access";
    data->mem_access_attr.attr.mode = 0664; //S_IRWXUGO; //S_IRUSR | S_IWUSR;
    data->mem_access_attr.read = mxt_mem_access_read;
    data->mem_access_attr.write = mxt_mem_access_write;
    data->mem_access_attr.size = data->mem_size;

    error = misc_register(&mxt_device);
    if (error) {
        dev_err(&data->client->dev, "Failure %d register miscdevice\n", error);
        goto err_remove_sysfs_group;
    }

    error = sysfs_create_bin_file(&data->client->dev.kobj, &data->mem_access_attr);
    if (error < 0) {
        dev_err(&data->client->dev, "Failed to create %s\n",
...
#ifdef MXT_HAS_EARLYSUSPEND
    mxt_configure_sleep(data);
#endif

    /* for misc open and close */
    mxt_data = data;
    dev_info(&data->client->dev, "probe successful\n");
    return 0;

err_deregister_misc:
    misc_deregister(&mxt_device);
...
static int mxt_suspend(struct device *dev)
{
    ...
    if (NULL == data) {
        printk(KERN_ERR"%s: data is NULL, line %d\n", __func__, __LINE__);
        return 0;
    }
    ...
    if (NULL == input_dev) {
        printk(KERN_ERR"%s: input_dev is NULL, line %d\n", __func__, __LINE__);
        return 0;
    }
...
#if (!defined(MXT_HAS_EARLYSUSPEND) || (!defined(CONFIG_FB) && !defined(CONFIG_HAS_EARLYSUSPEND)))
