...
#include <linux/mutex.h>//add by liwei
...
static struct mutex timerirq_mutex;
...
static int start_timerirq(struct timerirq_data *data)
{
	...
	pr_info("YLLOG:SENSOR:%s:,current->pid:%d\n", __func__, current->pid);
...
static int timerirq_open(struct inode *inode, struct file *file)
{
	...
	struct miscdevice *dev_data = NULL;
	struct timerirq_data *data = NULL;
	mutex_lock(&timerirq_mutex);//add by liwei
	dev_data = file->private_data;
	data = kzalloc(sizeof(*data), GFP_KERNEL);
	pr_info("YLLOG:SENSOR:%s:,current->pid:%d\n", __func__, current->pid);
	if (!data)
	{
		mutex_unlock(&timerirq_mutex);
		return -ENOMEM;
	}
	data->dev = dev_data;
...
static int timerirq_release(struct inode *inode, struct file *file)
{
	...
	if (!data)
	{
		mutex_unlock(&timerirq_mutex);//add by liwei
		return -EFAULT;
	}
	dev_dbg(data->dev->this_device, "timerirq_release\n");
	if (data->run)
	{
		pr_info("YLLOG:SENSOR:%s:,current->pid:%d_1\n", __func__, current->pid);
		data->run = false;
		data->data_ready = 0;
		del_timer_sync(&data->timer);	
		wake_up_interruptible(&data->timerirq_wait);
		complete(&data->timer_done);
		pr_info("YLLOG:SENSOR:%s:,current->pid:%d_2\n", __func__, current->pid);
	}
	kfree(data);
			   ...
			   unsigned int cmd, unsigned long arg)
{
	...
	pr_info("YLLOG:SENSOR:%s:,current->pid:%d,cmd:%d\n", __func__, current->pid, cmd);
