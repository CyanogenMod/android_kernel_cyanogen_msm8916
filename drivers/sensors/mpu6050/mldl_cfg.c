...
*NOTE:
*   software revision 0 falls back to the old derection method
*   based off the product version and product revision per the
*   table above
*/
static struct prod_rev_map_t sw_rev_map[] = {
	{0,	0,	0,	0},
	{1,MPU_SILICON_REV_B1,	131,8192},/*recv C*/
	{2,MPU_SILICON_REV_B1,	131,16384}/*recv D*/
};
/**
 *  @internal
		...
		struct mldl_cfg *mldl_cfg, void *mlsl_handle)
{
	...
	struct prod_rev_map_t rev;
	struct prod_rev_map_t *p_rev = &rev;
	unsigned sw_rev;
	unsigned short key;
	unsigned char bank =
	...
	if(sw_rev == 0)
	{
		...
		if(key == 0)
		{
			MPL_LOGE("Product id read ad 0"
				"indicates device is either"
				"incompatible or an MPU3050\n");

			p_rev->silicon_rev = MPU_SILICON_REV_B1;
			p_rev->gyro_trim = 131;
			//p_rev->accel_trim = 16384;
			p_rev->accel_trim = 8192;
			...
			/*check MPL is compiled for the device*/
...
int inv_mpu_set_power(struct mldl_cfg *mldl_cfg,
			void *gyro_handle,
			void *accel_handle,
			void *compass_handle,
			void *pressure_handle,
			int powerflag)
{
	...
	if(powerflag==false){
		...
		if(!(reg & BIT_SLEEP)) {
			result = inv_serial_single_write(gyro_handle, mldl_cfg->mpu_chip_info->addr,
		...
		pr_info("suspend end,MPU_PWR_MGMT status:0x%x",reg|BIT_SLEEP);
	}else{
		result = inv_serial_read(gyro_handle, mldl_cfg->mpu_chip_info->addr,
					 MPUREG_PWR_MGMT_1, 1, &reg);
		if (result) {
		...
		pr_info("resume  start,MPU_PWR_MGMT status:0x%x",reg & ~BIT_SLEEP);	
	}	
	return 0;
}
