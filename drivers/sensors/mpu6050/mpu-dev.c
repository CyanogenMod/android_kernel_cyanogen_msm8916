...
struct mpu_private_data {
	...
	int compass_status;/*add by liwei ,for  detecting compass error*/
	int compass_address;
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	struct ext_slave_platform_data **pdata_slave = mldl_cfg->pdata_slave;
	int i = 0;
    struct SensorCalInfo info;
	unsigned int cal_status = 0;
	short accel_bias[3]={0};
	...
	for (i = 0; i < EXT_SLAVE_NUM_TYPES; i++) {
		if (!pdata_slave[i])
			slave_adapter[i] = NULL;
		else
			slave_adapter[i] =
				i2c_get_adapter(pdata_slave[i]->adapt_num);
	}
	slave_adapter[EXT_SLAVE_TYPE_GYROSCOPE] = client->adapter;
	...
	case MPU_GET_I2C_SLAVES_ENABLED:
		if (copy_to_user(
			...
			retval = -EFAULT;
		break;
	case MPU_SET_ACCEL_BIAS:
		/*write acc bias from parameter partition*/
		if (copy_from_user(accel_bias, (unsigned char __user *)arg, 
				sizeof(accel_bias))) {
			dev_err(&client->adapter->dev,
				"%s: set accel bias faild, cmd %x, arg %lu\n", __func__, cmd, arg);
			retval = -EFAULT;
		}
		else {
		/*save 15 valid data bit and 1 signal symbol bit. 
		  The symbol bit 0 shows positive
		  The symbol bit 1 shows negative*/
			for (i = 0; i < 3; i++) {
				if (accel_bias[i] > 0) {
					info.value[2*i] = accel_bias[i]&0x00ff;
					//make sure the 15th bit is 0
					info.value[2*i+1] = ((accel_bias[i]&0x7f00)>>8);
				}
				else {
					info.value[2*i] = accel_bias[i]&0x00ff;
					//make sure the 15th bit is 0
					info.value[2*i+1] = ((accel_bias[i]&0x7f00)>>8)|0x80;
				}
			}
			pr_info("YLLOG:SENSOR:set acc__ bias,x:%13d,y:%13d,z:%13d",
				accel_bias[0],accel_bias[1],accel_bias[2]);
			info.SensorCalStatus = 1;
			if (sizeof(info) != sensparams_write_to_flash(SENSPARAMS_TYPE_ACCEL, 
		...
		break;
	case MPU_GET_ACCEL_BIAS:
		//save 15 valid data bit and 1 signal symbol bit. 
		//The symbol bit 0 shows positive
		//The symbol bit 1 shows negative 
		if (sizeof(info) == sensparams_read_from_flash(SENSPARAMS_TYPE_ACCEL, \
            (unsigned char *)&info, sizeof(info))) {
			for (i = 0; i < 3; i++) {
				accel_bias[i] = \
					((info.value[2*i+1]<<8)|(info.value[2*i]));
			}
			pr_info("YLLOG:SENSOR:get acc__ bias,x:%13d,y:%13d,z:%13d",
				accel_bias[0],accel_bias[1],accel_bias[2]);
		}
		else {
			pr_info("YLLOG:SENSOR:get acc__ bias faild, use default 0");
		}
		/*read acc bias from parameter partition*/
		if (copy_to_user((unsigned char __user *)arg, accel_bias, 
				sizeof(accel_bias))) {
			retval = -EFAULT;
		}
		break;
	case MPU_GET_CAL_STATUS:
		if (sizeof(info) == sensparams_read_from_flash(SENSPARAMS_TYPE_ACCEL, \
            (unsigned char *)&info, sizeof(info))) {
			cal_status = info.SensorCalStatus;
		}
		if (copy_to_user((unsigned int __user *)arg, &cal_status, sizeof(cal_status))) {
			retval = -EFAULT;
		}
		break;
	default:
...
int mpu_dev_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	inv_mpu_set_power(mldl_cfg,
		...
		false);
	mutex_unlock(&mpu->mutex);
	if(mldl_cfg->pdata->pm_suspend)
		mldl_cfg->pdata->pm_suspend();
	return 0;
}
...
int mpu_dev_resume(struct i2c_client *client)
{
	...
	struct ext_slave_platform_data **pdata_slave = mldl_cfg->pdata_slave;
	int ii;
	if(mldl_cfg->pdata->pm_resume){
		mldl_cfg->pdata->pm_resume();
	}
	for (ii = 0; ii < EXT_SLAVE_NUM_TYPES; ii++) {
		if (!pdata_slave[ii])
			slave_adapter[ii] = NULL;
			...
			struct ext_slave_descr *(*get_slave_descr)(void))
{
	...
	pr_info("%s: +%s Type %d: Addr: %2x IRQ: %2d, Adapt: %2d\n",
		__func__,
		slave_descr->name,
		slave_descr->type,
		slave_pdata->address,
...
#ifdef CONFIG_OF
static struct of_device_id mpu_match_table[] = {
	{ .compatible = "mpu6050"/*"Invensense,MPU6050"*/,},
	{ },
};
#else
#define mpu_match_table NULL
#endif
...
int mpu_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	/*liwei:for test i2c OK*/
#if 1
	res = i2c_smbus_read_byte_data(client,MPUREG_PRODUCT_ID);
	pr_info("YLLOG:SENSOR:-mpu_test i2c:0x%x",res);
#endif
	...
	res = mpu_parse_dt(&client->dev, pdata);
	if (res) {
